...
async function promptCredentials() {
  ...
  const password = await rl.question(
    "Staff password (will be visible as you type): ",
  );
  rl.close();
...
async function migrateAvatars(supabase) {
  ...
  console.log(
    `Avatars: ${migrated} migrated, ${skipped} skipped (already new path or external), ${failed} failed`,
  );
}
...
async function migrateDevotions(supabase) {
  ...
  for (const row of rows) {
    try {
      ...
      migrated++;
      console.log(
        `  ✓ lamb_devotion ${row.id}: ${urlMap.size} image(s) -> ${newPrefix}/`,
      );
    } catch (err) {
      failed++;
  ...
  console.log(
    `Devotions: ${migrated} migrated, ${skipped} skipped (no old-path images), ${failed} failed`,
  );
}
...
async function main() {
  ...
  console.log(
    "This project requires a logged-in staff session to migrate data (RLS).",
  );
  ...
  console.log(
    "remove the old profile/seedbkk/ and devotion/ folders from Storage yourself.",
  );
}
