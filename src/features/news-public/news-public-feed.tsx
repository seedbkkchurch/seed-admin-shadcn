...
import { useCanWriteNews, usePublicNewsFeed } from "@/features/news/data/queries";
...
// หน้าข่าว public เพียงหน้าเดียว ให้ทั้งคนไม่ login และคน login ใช้ร่วมกัน
// (ตอนแรกมี /_authenticated/news แยกต่างหาก แต่ _authenticated เป็น
// pathless layout จึงชนกับ path "/news" ตรงนี้พอดี — TanStack Router ฟ้อง
// "Conflicting configuration paths" ตอน dev server สตาร์ท จึงยุบมาเป็นหน้า
// เดียว ไม่มี route แยกฝั่ง authenticated อีกต่อไป — ดู grill-me follow-up
// 2026-08-25 bug fix) ปุ่มเขียน/จัดการข่าวโชว์เฉพาะคนมี news:write เท่านั้น
// (useCanWriteNews เช็ค auth ภายในตัวเองอยู่แล้ว คน anon ได้ false เฉยๆ
// ไม่ error) อ่านจาก DB view public_news_feed (กรอง status='published'
// ให้แล้ว) เหมือนเดิม
export function NewsPublicFeed() {
  const { data: entries, isPending, isError, error } = usePublicNewsFeed();
      ...
      <Main className="flex flex-1 flex-col gap-4 sm:gap-6">
        ...
        <div className="mx-auto grid w-full max-w-3xl gap-4">
          {isError ? (
            ...
            entries.map((entry) => {
              const authorName =
                entry.author_nick_name ?? entry.author_first_name ?? "ไม่ทราบชื่อ";
              const avatarUrl = entry.author_profile_picture ?? null;
